...
import type { SpatialDocument, SpatialObject } from './types/spatial.js';
 ...
 */
export function validateDocument(document: SpatialDocument): ValidationResult {
  const errors: string[] = [];
  ...
  const objectById = new Map<string, SpatialObject>();
  const objectsByType = new Map<string, SpatialObject[]>();
 ...
 * @param document The document to validate
 */
export function validateAndReport(document: SpatialDocument): string {
