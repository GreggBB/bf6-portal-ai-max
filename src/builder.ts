...
import { Elevations, type FloorAssetType } from './types/assets.js';
import { vec3 } from './utils/vector.js';
import { resetIdCounter, generateCombatPolygonId, IdPrefixes } from './utils/id.js';
import {
  generateFloorGrid,
  type FloorGridConfig,
  type FloorGridResult,
} from './generators/floor-grid.js';
import {
  generateSpawnSystem,
  generateTeamSpawns,
  type SpawnSystemConfig,
  type SpawnSystemResult,
} from './generators/spawn-system.js';
import {
  generateCombatArea,
  type CombatAreaConfig,
  type CombatAreaResult,
} from './generators/combat-area.js';
...
export class SpatialBuilder {
  ...
  private targetMap: string = 'MP_Abbasid';
  private floorGrid: FloorGridResult | null = null;
  private team1Spawns: SpawnSystemResult | null = null;
  private team2Spawns: SpawnSystemResult | null = null;
  private combatArea: CombatAreaResult | null = null;
  private sector: Sector | null = null;
  private hasDeployCam: boolean = false;
   ...
   */
  addFloorGrid(config: Omit<FloorGridConfig, 'origin'> & { origin?: Vector3 }): this {
    const fullConfig: FloorGridConfig = {
  ...
  addDeployCam(options: DeployCamOptions = {}): this {
    ...
    const deployCam = generateDeployCamOverPlatform(
      this.floorGrid.center,
      this.floorGrid.bounds.min.y
    );
 ...
 */
export function createBuilder(): SpatialBuilder {
