...
import { FloorAssets, DefaultFloorTile, type FloorAssetType } from '../types/assets.js';
 ...
 */
export function generateFloorGrid(config: FloorGridConfig): FloorGridResult {
  const {
    origin,
    tilesX,
    tilesZ,
    elevation = 90,
    assetType = 'FiringRange_Floor_A',
    idPrefix = IdPrefixes.Floor,
  } = config;
  ...
  for (let xIdx = 0; xIdx < tilesX; xIdx++) {
    for (let zIdx = 0; zIdx < tilesZ; zIdx++) {
      const position = vec3(
        origin.x + xIdx * asset.spacingX,
        elevation,
        origin.z + zIdx * asset.spacingZ
      );

      const tile: SpatialObject = {
        name: asset.type,
        type: asset.type,
...
export function calculateGridSize(
  tilesX: number,
  tilesZ: number,
  assetType: FloorAssetType = 'FiringRange_Floor_A'
